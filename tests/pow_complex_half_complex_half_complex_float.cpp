...
void test_pow() {
  ...
  complex<float> in1_host_trunc = trunc_float(in1_host);
  sycl::ext::cplx::complex<sycl::half> in1_device{0.42, 0.0};
  complex<float> out2_host{};
  auto *out2_device = sycl::malloc_shared<sycl::ext::cplx::complex<sycl::half>>(1, Q);
  { out2_host = pow(in0_host_trunc, in1_host_trunc); }
  Q.single_task([=]() { out2_device[0] = sycl::ext::cplx::pow<sycl::half>(in0_device, in1_device); }).wait();
  {
    if (!almost_equal(trunc_float(out2_host), sycl_half_to_float(out2_device[0]), precision)) {
      std::cerr << std::setprecision(std::numeric_limits<float>::max_digits10) << "Host: " << trunc_float(out2_host) << " GPU: " << sycl_half_to_float(out2_device[0]) << std::endl;
      std::exit(112);
    }
  }
}
