...
TEMPLATE_TEST_CASE("Test complex conj cmplx", "[conj]", double, float,
  ...
  if (is_type_supported<T>(Q)) {
    Q.single_task([=]() {
       d_cplx_out[0] = sycl::ext::cplx::conj<T>(cplx_input);
     }).wait();
  ...
  h_cplx_out = sycl::ext::cplx::conj<T>(cplx_input);
...
TEMPLATE_TEST_CASE("Test complex conj deci", "[conj]",
  ...
  if (is_type_supported<T>(Q) && is_type_supported<X>(Q)) {
    Q.single_task([=]() {
       d_cplx_out[0] = sycl::ext::cplx::conj<X>(input);
     }).wait();
    Q.copy(d_cplx_out, &h_cplx_out, 1).wait();
  ...
  // Check cplx::complex output from host
  h_cplx_out = sycl::ext::cplx::conj<X>(input);
