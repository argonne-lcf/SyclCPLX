...
void test_atanh(){
  ...
  Q.single_task([=]() {
   out1_device[0] = sycl::ext::cplx::atanh(in0_device);
   }).wait();
   std::complex<float> out1_device_std = { out1_device[0].real(), out1_device[0].imag() };
   {
     if ( !almost_equal(tanh(out1_device_std), in0_host, 2*precision) ) {
          std::cerr << std::setprecision (std::numeric_limits<float>::max_digits10 )
                    << "Expected:" << in0_host << " Got: " << tanh(out1_device_std) << std::endl;
          std::exit(112);
     }
   }
}
