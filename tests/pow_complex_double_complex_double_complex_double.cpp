...
void test_pow() {
  ...
  Q.single_task([=]() { out2_device[0] = sycl::ext::cplx::pow<double>(in0_host, in1_host); }).wait();
  {
    if (!almost_equal(out2_host, out2_device[0], precision)) {
      std::cerr << std::setprecision(std::numeric_limits<double>::max_digits10) << "Host: " << out2_host << " GPU: " << out2_device[0] << std::endl;
      std::exit(112);
    }
  }
}
