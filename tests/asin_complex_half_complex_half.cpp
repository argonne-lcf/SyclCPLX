...
void test_asin() {
  ...
  complex<float> x_host{0.42, 0.23};
  complex<float> x_host_trunc = trunc_float(x_host);
  sycl::ext::cplx::complex<sycl::half> x_device{0.42, 0.23};
  complex<float> o_host{};
  auto *o_device = sycl::malloc_shared<sycl::ext::cplx::complex<sycl::half>>(1, Q);
  { o_host = asin(x_host_trunc); }
  Q.single_task([=]() { o_device[0] = sycl::ext::cplx::asin<sycl::half>(x_device); }).wait();
  {
    if (!almost_equal(trunc_float(o_host), sycl_half_to_float(o_device[0]), precision)) {
      std::cerr << std::setprecision(std::numeric_limits<float>::max_digits10) << "Host: " << trunc_float(o_host) << " GPU: " << sycl_half_to_float(o_device[0]) << std::endl;
      std::exit(112);
    }
  }
}
