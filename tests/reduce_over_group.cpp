...
void test_reduce_over_group(sycl::queue q, T input, BinaryOperation binary_op) {
  using V = typename T::value_type;

  constexpr size_t N = input.size();

  const auto init =
      sycl::ext::cplx::cplex::detail::get_init<V, BinaryOperation>();
  ...
  auto *h_output_with_init = sycl::malloc_device<V>(1, q);
  auto *h_output_without_init = sycl::malloc_device<V>(1, q);
  ...
  q.submit([&](sycl::handler &cgh) {
    cgh.parallel_for(sycl::nd_range<1>(N, N), [=](sycl::nd_item<1> it) {
      ...
      *d_output_with_init =
          sycl::ext::cplx::reduce_over_group(g, d_in[gid], init, binary_op);
      *d_output_without_init =
          sycl::ext::cplx::reduce_over_group(g, d_in[gid], binary_op);
    });
  });
  q.copy(d_output_with_init, h_output_with_init, 1);
  q.copy(d_output_without_init, h_output_without_init, 1);
...
TEMPLATE_TEST_CASE_SIG("Test complex reduce_over_group", "[reduction]",
  ...
  if (is_type_supported<T>(q)) {
    test_reduce_over_group(q, test_cases, binary_op);
  }
}
...
TEMPLATE_TEST_CASE_SIG("Test marray<complex> reduce_over_group", "[reduction]",
                       ((typename T, std::size_t N, typename BinaryOperation),
                        T, N, BinaryOperation),
                       (double, 4, sycl::plus<>), (float, 4, sycl::plus<>),
                       (sycl::half, 4, sycl::plus<>)) {
