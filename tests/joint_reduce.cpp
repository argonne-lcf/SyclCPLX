...
void test_joint_reduce(sycl::queue q, T input, BinaryOperation binary_op) {
...
TEMPLATE_TEST_CASE_SIG("Test complex joint_reduce", "[reduction]",
  ...
  if (is_type_supported<T>(q)) {
    test_joint_reduce(q, test_cases, binary_op);
  }
}
...
TEMPLATE_TEST_CASE_SIG("Test marray<complex> joint_reduce", "[reduction]",
                       ((typename T, std::size_t N, typename BinaryOperation),
                        T, N, BinaryOperation),
                       (double, 4, sycl::plus<>), (float, 4, sycl::plus<>),
                       (sycl::half, 4, sycl::plus<>)) {
