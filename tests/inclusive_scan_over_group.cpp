...
void test_inclusive_scan_over_group(sycl::queue q, T input,
  ...
  q.submit([&](sycl::handler &cgh) {
    cgh.parallel_for(sycl::nd_range<1>(N, N), [=](sycl::nd_item<1> it) {
      ...
      d_output_with_init[lid] = sycl::ext::cplx::inclusive_scan_over_group(
          g, d_in[gid], binary_op, init);
      d_output_without_init[lid] =
          sycl::ext::cplx::inclusive_scan_over_group(g, d_in[gid], binary_op);
    });
  });
...
TEMPLATE_TEST_CASE_SIG("Test complex inclusive_scan_over_group", "[scan]",
  ...
  if (is_type_supported<T>(q)) {
    test_inclusive_scan_over_group(q, test_cases, binary_op);
  }
}
...
TEMPLATE_TEST_CASE_SIG("Test marray<complex> inclusive_scan_over_group",
                       "[scan]",
                       ((typename T, std::size_t N, typename BinaryOperation),
                        T, N, BinaryOperation),
                       (double, 4, sycl::plus<>), (float, 4, sycl::plus<>),
