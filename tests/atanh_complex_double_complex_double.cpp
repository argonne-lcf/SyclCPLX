...
void test_atanh(){
  ...
  Q.single_task([=]() {
   out1_device[0] = sycl::ext::cplx::atanh<double>(in0_host);
   }).wait();
   {
     if ( !almost_equal(tanh(out1_device[0]), in0_host, 2*precision) ) {
          std::cerr << std::setprecision (std::numeric_limits<double>::max_digits10 )
                    << "Expected:" << in0_host << " Got: " << tanh(out1_device[0]) << std::endl;
          std::exit(112);
     }
   }
}
